...
namespace BitcoinAddressesGenerator
{
    public partial class FormMain : Form
    {
        ...
        private (string privateUnCompressedKeyString, string addressUnCompressed, string privateCompressedKeyString, string hexCompressed, string addressCompressed ) CreateKeyAddress(bool isSegwit, bool isSegwitP2SH, string delem)
        {
            //вначале получим сжатый ключ, извлечем из него bytes и на их основе создадим несжатый
            var privateCompressedKey = new Key(fCompressedIn: true);

            var bytes = privateCompressedKey.ToBytes();

            var bitcoinPrivateCompressedKey = privateCompressedKey.GetWif(CurrentNetwork);

            string addressCompressed = bitcoinPrivateCompressedKey.GetAddress(ScriptPubKeyType.Legacy).ToString();

            if (isSegwit)
            {
                addressCompressed += delem + bitcoinPrivateCompressedKey.GetAddress(ScriptPubKeyType.Segwit).ToString();
            }
            if (isSegwitP2SH)
            {
                addressCompressed += delem + bitcoinPrivateCompressedKey.GetAddress(ScriptPubKeyType.SegwitP2SH).ToString();
            }

            var privateUnCompressedKey = new Key(bytes, fCompressedIn: false);
            var bitcoinPrivateUnCompressedKey = privateUnCompressedKey.GetWif(CurrentNetwork);
            string addressUnCompressed = bitcoinPrivateUnCompressedKey.GetAddress(ScriptPubKeyType.Legacy).ToString();
            if (isSegwit)
            {
                addressUnCompressed += delem + bitcoinPrivateUnCompressedKey.GetAddress(ScriptPubKeyType.Segwit).ToString();
            }
            if (isSegwitP2SH)
            {
                addressUnCompressed += delem + bitcoinPrivateUnCompressedKey.GetAddress(ScriptPubKeyType.SegwitP2SH).ToString();
            }

            return (bitcoinPrivateUnCompressedKey.ToString(), addressUnCompressed, bitcoinPrivateCompressedKey.ToString(), privateCompressedKey.ToHex(), addressCompressed);
        }
        private string CreateKeyAddresses(bool isCompressed, bool isUnCompressed, bool isSegwit, bool isSegwitP2SH, string delem, string format)
        {
            ...
            {
                //0 - private key сжатый, 1 - Hex Private Key, 2 - addresses сжатый, 3- delimitator, 4 - private key несжатый, 5 - addresses несжатый
                //format = "{0}{3}{1}{3}{2}{3}{4}{3}{5}";

                (string keysUc, string addrUc, string keysC, string hexC, string addrC) = CreateKeyAddress(isSegwit, isSegwitP2SH, delem);
                return string.Format(format, keysC, hexC, addrC, delem, keysUc, addrUc);
        ...
        private string GetOutputFormat()
        {
            ...
            var both = radioButtonBothKey.Checked;
            //var Segwit = checkBoxSegwit.Checked;
            //var SegwitP2SH = checkBoxSegwitP2SH.Checked;

            if (both) //тут Hex одинаков
            {
                //0 - private key WIF сжатый, 1 - Hex Private Key, 2 - addresses сжатый, 3- delimitator, 4 - private key WIF несжатый, 5 - addresses несжатый
                format = "{0}{3}{1}{3}{2}{3}{4}{3}{5}"; 
            }
            if (!WIFKey)
            {
                format = format.Replace("{0}{3}", "").Replace("{4}{3}", ""); //удаляем WIF
            }
            if (!HexPrivateKey)
            {
                format = format.Replace("{1}{3}", ""); //удаляем Hex
            }
